...
if __name__ == '__main__':
    ...
    is_load = False
    ckpt_path = './ACGAN+_Samples/checkpoint_iteration_5000.tar'

    # Root directory for dataset
    data_path = '/dataset/celeba_m'
    ...
    for epoch in range(last_epoch, max_epochs):
        ...
        for i, (real_img, real_c) in enumerate(dataloader, str_i):
            ...
            v, c = net_D(real_img)
            loss_real = (- torch.mean(v) + CE(c, real_c)) * 0.5
            v, c = net_D(fake_img.detach())
            loss_fake = (torch.mean(v) + CE(c, fake_c)) * 0.5
            ...
            if i % n_critic == 0:
                ...
                loss_G = (- torch.mean(v) + CE(c, fake_c)) * 0.5
